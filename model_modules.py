...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        norm="batch",
        do_activation=True,
    ):  # bias default is True in Conv2d
...
class Generator(nn.Module):
    def __init__(
        ...
        # 8-step UNet decoder
        self.decoder1 = DecoderBlock(512 * 1, 512, bias=bias, norm=norm)
        self.decoder2 = DecoderBlock(
            1024, 512, bias=bias, norm=norm, dropout_prob=dropout_prob
        )
        self.decoder3 = DecoderBlock(
...
class Discriminator(nn.Module):
    def __init__(
        self, in_channels=3, out_channels=1, bias=False, norm="batch", sigmoid=True
    ):
        super(Discriminator, self).__init__()
        self.sigmoid = sigmoid

        # 70x70 discriminator
        self.disc1 = EncoderBlock(
            in_channels * 2, 64, bias=bias, do_norm=False, do_activation=False
        )
...
class GeneratorJohnson(nn.Module):
    ...
    def __init__(
        self, in_channels=3, out_channels=3, do_norm=True, norm="batch", bias=True
    ):
        ...
        model += [
            ...
            # u32
            nn.Conv2d(
                32, out_channels, (7, 7), padding=3, stride=1, bias=bias
            ),  # c7s1-3
            nn.Tanh(),
        ]
...
class Resnet50(nn.Module):
    ...
    def __init__(self, image_channel=3, norm="instancenorm"):
        ...
        model += [nn.ReflectionPad2d(3), nn.Conv2d(64, image_channel, 7), nn.Tanh()]
